...
export function Sidebar() {
  ...
  return <motion.aside initial={false} animate={{
    ...
    ease: "easeInOut"
      ...
      {/* Navigation */}
      <nav className="flex-1 py-6 px-3 space-y-1 border-secondary">
        {navItems.map(item => {
        ...
        return <Link key={item.path} to={item.path}>
            ...
            scale: 0.98
          }} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200", isActive ? "bg-sidebar-primary text-sidebar-primary-foreground shadow-glow" : "text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground")}>
                ...
                opacity: 0,
                width: 0
              }} className="font-medium text-sm whitespace-nowrap overflow-hidden text-secondary">
                      {item.label}
                    </motion.span>}
                </AnimatePresence>
              </motion.div>
      ...
      <div className="p-3 border-t border-sidebar-border">
        <Button variant="ghost" size="sm" onClick={toggle} className="w-full justify-center text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent">
          {collapsed ? <ChevronRight className="w-4 h-4" /> : <>
              <ChevronLeft className="w-4 h-4 mr-2 border-primary" />
              <span className="text-sm text-secondary">Collapse</span>
            </>}
        </Button>
      </div>
    </motion.aside>;
